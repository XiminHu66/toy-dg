...
核心循环：**营地配装 → 卡牌战斗 → 事件检定 → 拾取／装包 → 深入或撤离 → 出售／强化 → 再次出发**。
...
建议桌面1440×900。新增原创AI二次元角色与遗迹背景；敌人暂为代码绘制的符文造型。
...
验证卡牌循环、能量与目标合法性、伤害预览、破韧与格挡、背包操作守恒、交易与强化、失败损失、存档与随机数恢复，以及6组种子的完整探索流程。UI冒烟检查覆盖营地三个分页、战斗、事件、掉落与出口，并检查页面边界。旧版存档自动迁移，第一次读取时另存profile.json.v1备份。自动验证不代替人工平衡试玩。
