...
func _draw() -> void:
	...
	for i in range(session.enemies.size()):
		...
		text_at(Vector2(center.x,35),msg,Color("f5b49e") if int(intent.hits)>0 else Color("8bd1d0"),20)
		text_at(Vector2(center.x,56),"失衡：本次伤害减半" if target.broken else "下一步意图",Color("b1b7c8"),12)
